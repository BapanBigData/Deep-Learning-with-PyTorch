...
def plot_training_history_updated(train_losses, train_accuracies, test_losses, test_accuracies):
    """
    Plots training history for loss and accuracy with an X-axis interval of 5 epochs.
    Adjusts automatically for early stopping.
    """
    # Determine the number of epochs completed based on the data
    completed_epochs = len(train_losses)

    # Create figure
    plt.figure(figsize=(12, 6))

    # Define X-axis ticks at intervals of 5 (or fewer if training stopped early)
    x_ticks_interval = max(1, completed_epochs // 10)  # At least one tick every 10% of epochs
    x_ticks = range(1, completed_epochs + 1, x_ticks_interval)

    # Subplot for training and testing loss
    plt.subplot(1, 2, 1)
    plt.plot(range(1, completed_epochs + 1), train_losses, label='Train Loss', linewidth=2.5, linestyle='-', color='blue')
    plt.plot(range(1, completed_epochs + 1), test_losses, label='Test Loss', linewidth=2.5, linestyle='--', color='orange')
    plt.xlabel('Epoch', fontsize=12)
    plt.ylabel('Loss', fontsize=12)
    plt.title('Training and Testing Loss', fontsize=14, fontweight='bold')
    plt.legend(fontsize=10)
    ...
    # Subplot for training and testing accuracy
    plt.subplot(1, 2, 2)
    plt.plot(range(1, completed_epochs + 1), train_accuracies, label='Train Accuracy', linewidth=2.5, linestyle='-', color='green')
    plt.plot(range(1, completed_epochs + 1), test_accuracies, label='Test Accuracy', linewidth=2.5, linestyle='--', color='red')
    plt.xlabel('Epoch', fontsize=12)
    plt.ylabel('Accuracy', fontsize=12)
    plt.title('Training and Testing Accuracy', fontsize=14, fontweight='bold')
    plt.legend(fontsize=10)
